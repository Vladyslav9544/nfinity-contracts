...
/// @title Nfinity: a simple decentralized ticket store on Ethereum
/// @notice Final project for ConsenSys Academy's Developer Bootcamp 2019.
contract Nfinity is Ownable, ReentrancyGuard {
    ...
    event CustomerCheckedIn(
        uint indexed _eventId,
        uint indexed _purchaseId,
        address indexed _by
    );
    ...
    modifier validEventId(uint _eventId) {
        require(
            _eventId <= store.counterEvents.current,
    ...
    modifier validPurchaseId(uint _purchaseId) {
        require(
            _purchaseId <= store.counterPurchases.current,
    ...
    modifier onlyOwnerOrOrganizer(uint _eventId) {
        require(
            msg.sender == owner() || msg.sender == events[_eventId].organizer,
    ...
    function createEvent(
        ...
        require(
            _storeIncentive >= 0 && _storeIncentive <= 10000,
        ...
        onlyOrganizer(_eventId)
    {
        ...
        emit EventSalesStarted(
            _eventId,
            events[_eventId].externalId,
            msg.sender
        );
    }
    ...
    function settleEvent(
        uint _eventId
    ) external nonReentrant storeOpen onlyOwner {
        require(
            events[_eventId].status == EventStatus.Completed,
        ...
        uint _storeIncentiveBalance = _eventBalance.mul(_storeIncentive).div(
            10000
        );
        ...
        returns (uint purchaseId)
    {
        ...
        require(
            _quantity <= events[_eventId].ticketsLeft,
        ...
        require(
            bytes(_customerId).length != 0,
            "ERROR-029: customer ID cannot be empty in order to proceed"
        );
        require(
            msg.value == _quantity.mul(events[_eventId].ticketPrice),
        ...
        purchases[purchaseId].customer = payable(msg.sender);
        purchases[purchaseId].customerId = keccak256(bytes(_customerId));
        purchases[purchaseId].total = _quantity.mul(
            events[_eventId].ticketPrice
        );
        events[_eventId].ticketsSold = events[_eventId].ticketsSold.add(
            _quantity
        );
        events[_eventId].ticketsLeft = events[_eventId].ticketsLeft.sub(
            _quantity
        );
        events[_eventId].eventBalance = events[_eventId].eventBalance.add(
            purchases[purchaseId].total
        );
        ...
        emit PurchaseCompleted(
            purchaseId,
            purchases[purchaseId].externalId,
            msg.sender,
            _eventId
        );
        return (purchaseId);
        ...
        purchaseCompleted(_purchaseId)
    {
        ...
        require(
            payable(msg.sender) == purchases[_purchaseId].customer,
        ...
        events[_eventId].ticketsCancelled = events[_eventId]
            .ticketsCancelled
            .add(purchases[_purchaseId].quantity);
        events[_eventId].ticketsLeft = events[_eventId].ticketsLeft.add(
            purchases[_purchaseId].quantity
        );
        events[_eventId].eventBalance = events[_eventId].eventBalance.sub(
            purchases[_purchaseId].total
        );
        events[_eventId].refundableBalance = events[_eventId]
            .refundableBalance
            .add(purchases[_purchaseId].total);
        store.refundableBalance = store.refundableBalance.add(
            purchases[_purchaseId].total
        );
        emit PurchaseCancelled(
            _purchaseId,
            purchases[_purchaseId].externalId,
            msg.sender,
            _eventId
        );
    }
        ...
        validPurchaseId(_purchaseId)
    {
        ...
        events[_eventId].ticketsRefunded = events[_eventId].ticketsRefunded.add(
            purchases[_purchaseId].quantity
        );
        events[_eventId].refundableBalance = events[_eventId]
            .refundableBalance
            .sub(purchases[_purchaseId].total);
        store.refundableBalance = store.refundableBalance.sub(
            purchases[_purchaseId].total
        );
    ...
    function getCountOrganizerEvents(
        address _organizer
    ) external view returns (uint countEvents) {
    ...
    /// @notice Get IDs of all events, according to its position on list of events created by an organizer.
    /// @param _organizer organizer address
    /// @return  eventIds IDs
    function getEventIdsByOrganizer(
        address _organizer
    ) external view returns (uint[] memory eventIds) {
        eventIds = organizerEvents[_organizer];
    }
    ...
    function getEventIdByIndex(
        address _organizer,
        uint _index
    ) external view returns (uint eventId) {
        require(
            organizerEvents[_organizer].length != 0,
            "ERROR-040: organizer has not created events yet"
        );
        require(
            _index < organizerEvents[_organizer].length,
    ...
    function getCountCustomerPurchases(
        address _customer
    ) external view returns (uint countPurchases) {
    ...
    /// @notice Get IDs of all purchases, according to its position on list of purchases performed by a customer.
    /// @param _customer customer address
    /// @return  purchaseIds IDs
    function getPurchaseIdsByCustomer(
        address _customer
    ) external view returns (uint[] memory purchaseIds) {
        purchaseIds = customerPurchases[_customer];
    }
    ...
    /// @param _index position in the list. Starting from zero
    // @return Purchase ID
    function getPurchaseIdByIndex(
        address _customer,
        uint _index
    ) external view returns (uint purchaseId) {
        require(
            customerPurchases[_customer].length != 0,
            "ERROR-043: customer has not purchased tickets yet"
        );
        require(
            _index < customerPurchases[_customer].length,
    ...
    function getOrganizerByIndex(
        uint _index
    ) external view returns (address organizer) {
        require(
            _index < organizers.length,
    ...
    function getCustomerByIndex(
        uint _index
    ) external view returns (address customer) {
        require(
            _index < customers.length,
